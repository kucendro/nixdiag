...
pub const WRITING: &str = "writing {path}";
pub const SCHEMA_MISMATCH: &str = "facts.json declares schema {found}, but nixdiag {version} implements schema {expected} — the module that produced these facts comes from a different nixdiag revision; pin `lib` and the binary to the same one";
...
pub const CONNECTION_ERROR: &str = "{host}/{unit} -> {target}: {reason}";
pub const UNKNOWN_HOST: &str = "unknown host `{host}`";
pub const NOT_ENABLED: &str = "`{unit}` is not enabled on `{host}`";
pub const AMBIGUOUS_UNIT: &str = "`{unit}` runs on several hosts ({hosts}); use host/{unit}";
pub const AMBIGUOUS_NAME: &str = "`{name}` is served by several units; use host/unit";
pub const NO_PORT: &str = "nothing on `{host}` listens on port {port}";
pub const UNKNOWN_TARGET: &str = "not a host, a unit, a declared name, a URL, `internet` or `lan`";
